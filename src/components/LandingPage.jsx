...
const LandingPage = ({signIn}) => {
    return (
        <div> <section className='Landingpage relative px-6  top-10 flex  flex-wrap flex-row-reverse justify-between items-center'>
            <div>
            ...
            <div>
                <h1 className=' headingpart text-3xl w-[30rem]  font-serif'><span className='text-red-300 font-semibold font-sans text-5xl'>Notify </span> is an <span className='text-red-300 font-sans '>Authenticated web App </span> where user can Create  private Notes and Save  Securely here</h1>
                <h2 className='text-sm font-medium text-green-600'>Your Notes are always  Secure. No One can See Your Notes 😄</h2>
            </div>
        </section>
            <button onClick={() => {
                signIn()
            }} className="btn bg-blue-600 font-normal cursor-pointer text-base relative bottom-12
